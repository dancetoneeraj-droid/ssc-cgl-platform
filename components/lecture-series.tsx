type LectureSeriesProps = {
  ...
  context: string;
  /**
   * Optional YouTube video IDs keyed by 1-indexed lecture number.
   * Slots without an ID render a "coming soon" placeholder.
   */
  videoIds?: Partial<Record<1 | 2 | 3, string>>;
};

export function LectureSeries({ context, videoIds }: LectureSeriesProps) {
  return (
    <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {([1, 2, 3] as const).map((n) => {
        const videoId = videoIds?.[n];
        return (
          <li key={n}>
